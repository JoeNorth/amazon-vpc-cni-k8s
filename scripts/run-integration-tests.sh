...
if [[ $RUN_CNI_INTEGRATION_TESTS == true ]]; then
    ...
    (cd "$INTEGRATION_TEST_DIR/custom-networking-sgpp" && CGO_ENABLED=0 ginkgo --skip="Trunk ENI Security Group" -v --timeout 60m --no-color --fail-on-pending -- --cluster-kubeconfig="$KUBECONFIG" --cluster-name="$CLUSTER_NAME" --aws-region="$AWS_DEFAULT_REGION" --aws-vpc-id="$VPC_ID" --ng-name-label-key="kubernetes.io/os" --ng-name-label-val="linux" --test-image-registry="$TEST_IMAGE_REGISTRY")
    TEST_PASS=$?
